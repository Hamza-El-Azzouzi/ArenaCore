...
export const CAPS=Object.freeze({sourceBytes:MAX_SOURCE_BYTES,outputBytes:256*1024,artifactBytes:8*1024*1024,artifactFiles:256,inputBytes:256*1024,maxCases:100,scratchBytes:32*1024*1024,pids:64,compileMs:15000,totalMs:30000,caseMs:10000,compileMemoryMiB:512,maxMemoryMiB:1024,maxFileBytes:8*1024*1024});
export const sandboxRequestSchema=z.strictObject({
  ...
  cases:z.array(z.strictObject({id:z.uuid(),input:z.string().refine(v=>Buffer.byteLength(v)<=CAPS.inputBytes)})).min(1).max(CAPS.maxCases),
  ...
  if(r.cases.reduce((n,t)=>n+Buffer.byteLength(t.input),0)>1024*1024)c.addIssue({code:'custom',path:['cases'],message:'Input budget exceeded'});
});
