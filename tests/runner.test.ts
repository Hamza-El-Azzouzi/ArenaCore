...
class Engine implements DockerTransport {
  calls:string[][]=[];containers=new Map<string,readonly string[]>();missingRuntime=false;unsafe=false;cleanupFails=false;oomState=false;execError?:DockerError;artifact?:Buffer;onExec?:()=>void;
  async command(args:readonly string[]):Promise<CommandResult> {
    ...
    if(args[0]==='inspect'){
      ...
      if(args.at(-1)==='{{json .State}}')return result(JSON.stringify({OOMKilled:this.oomState,Running:!this.oomState,Pid:this.oomState?0:123}));
