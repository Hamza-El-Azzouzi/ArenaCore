...
class Engine implements DockerTransport {
  calls:string[][]=[];containers=new Map<string,readonly string[]>();missingRuntime=false;unsafe=false;cleanupFails=false;oomState=false;stoppedState=false;execError?:DockerError;artifact?:Buffer;onExec?:()=>void;
  async command(args:readonly string[]):Promise<CommandResult> {
    ...
    if(args[0]==='inspect'){
      ...
      if(args.at(-1)==='{{json .State}}')return result(JSON.stringify({OOMKilled:this.oomState,Running:!this.oomState&&!this.stoppedState,Pid:this.oomState||this.stoppedState?0:123}));
...
class Metrics implements MetricsReader {
  calls=0;oom=false;terminal=false;
  async snapshot(){this.calls++;if(this.terminal&&this.calls%2===0)throw new Error('INVALID_CGROUP_PID');return {path:'/system.slice/docker-test.scope',cpuUsec:this.calls*1500,memoryPeakBytes:64*1024*1024,oomKills:this.oom&&this.calls%2===0?1:0};}
}
...
describe('isolated runner policy and supervisor failure boundaries',()=>{
  ...
  it('trusts Docker terminal OOM state when the killed cgroup no longer has a PID',async()=>{const engine=new Engine(),metrics=new Metrics();engine.oomState=true;metrics.terminal=true;const {supervisor}=await ready(engine,metrics);const observation=await supervisor.execute(request(),new AbortController().signal);expect(observation.cases[0]).toMatchObject({failure:'MEMORY_LIMIT_EXCEEDED'});expect(observation.cases[0]).not.toHaveProperty('cpuMs');expect(observation.cases[0]).not.toHaveProperty('memoryKiB');});
  it('preserves a completed result without metrics when a non-OOM sandbox has stopped',async()=>{const engine=new Engine(),metrics=new Metrics();engine.stoppedState=true;metrics.terminal=true;const {supervisor}=await ready(engine,metrics);const observation=await supervisor.execute(request(),new AbortController().signal);expect(observation.cases[0]).toMatchObject({stdout:'5\n',exitCode:0});expect(observation.cases[0]).not.toHaveProperty('failure');expect(observation.cases[0]).not.toHaveProperty('cpuMs');});
