ALTER TABLE "Competition" ADD COLUMN "ownerId" UUID;
ALTER TABLE "Execution" ADD COLUMN "competitionRoundId" UUID;

CREATE INDEX "Competition_ownerId_endsAt_idx" ON "Competition"("ownerId", "endsAt");
CREATE INDEX "Execution_competitionRoundId_userId_state_idx" ON "Execution"("competitionRoundId", "userId", state);
...
ALTER TABLE "Execution" ADD CONSTRAINT "Execution_competitionRoundId_fkey" FOREIGN KEY ("competitionRoundId") REFERENCES "CompetitionRound"(id) ON DELETE RESTRICT ON UPDATE CASCADE;
...
BEGIN
  ...
  IF (NEW.id, NEW."userId", NEW."problemVersionId", NEW."competitionRoundId", NEW.language, NEW.mode, NEW."sourceCode",
      NEW."payloadHash", NEW."idempotencyKey", NEW."createdAt", NEW."queueExpiresAt") IS DISTINCT FROM
     (OLD.id, OLD."userId", OLD."problemVersionId", OLD."competitionRoundId", OLD.language, OLD.mode, OLD."sourceCode",
