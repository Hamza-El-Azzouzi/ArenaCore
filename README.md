...
| 7. Trusted judging | Public Run / private Submit suites, external comparator, verdicts, sanitized diagnostics, aggregate metrics | Correct/incorrect solutions judged accurately; hidden output never leaks | In progress—live judging/privacy and cgroup metric host gates pass; expanded suite pending |
...
Current: the stage 6 runtime images, all 14 original dedicated-host gVisor isolation tests, the idle supervisor/janitor lifecycle drill, restricted worker dependency check, controlled seven-job live judging gate, abrupt supervisor-death recovery gate, and restricted-identity cgroup metric gate pass. The metric gate proved positive CPU/peak-memory measurement and terminal gVisor OOM classification with complete cleanup. Rerun the expanded 15-test isolation suite, then complete independent review before launch. Execution stays off until every runner and judging launch check passes.
