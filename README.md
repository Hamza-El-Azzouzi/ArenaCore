...
| 8. Frontend integration | Replace generated UI mocks with typed REST/Socket.IO adapter | End-to-end login, Run, Submit, cancel, reconnect and history pass | In progress—core flows, authoring, discussions and competitions connected; live regression pass pending |
| 9. Production operations | IaC, CI/CD, staging, secrets, telemetry, backups, restore, rollback, capacity and abuse limits | Security review, load test, restore and rollout drills pass | In progress—off-host backup and functional restore pass; timed recovery, monitoring, rollback and load drills pending |
...
Start with the [backend learning course](docs/learning/README.md): 84 separate chapters explaining NestJS, architecture, PostgreSQL, authentication, cryptography, testing, deployment, recovery, and public data projections through this project's code. Each topic includes mechanisms, concrete examples, failure cases, and exercises with worked reasoning. Follow the reading paths and finish with the NestJS practice labs. Implemented queue/realtime, supervisor and judging protocols are distinguished from pending live-host acceptance.
...
Problem authoring now records difficulty, an immutable `STDIN` or `FILES` input contract, editable starter templates for all supported languages, and public/hidden case files. The runner accepts only validated bounded filenames and extracts file inputs into the fresh per-case gVisor tmpfs. Public problem projections expose public example files only. The frontend problem pane owns its scroll area and renders common statement Markdown cleanly. Community event creation is a four-step wizard with server-backed problem search and multi-selection. Migration `20260926030000_problem_file_inputs` requires backend-first deployment; the four new dedicated-host file-input checks remain pending on the production runner.

Problem creation and editing now share a five-step administrator workflow. Edits create immutable versions, drafts do not replace the live version, and publication uses the explicit current-version pointer. Administrators can publish, unpublish, edit, and safely delete unused drafts, plus publish, unpublish, edit, or delete competitions when history permits. Community competition owners have an organizer workspace for registrations, safe competition submissions, rankings, and pre-start edits; every owner route rechecks `ownerId` on the server.
...
- Problem discussion implementation: added visible/hidden/deleted moderation state, bounded threads and one-level replies, explicit author projections, deterministic cursor pagination, idempotent likes, audited creation and a transactional ten-writes-per-minute user quota shared by all API replicas. Public reads expose no provider or session identity. The real discussion screen supports signed-out reading, authenticated posting/replies/likes and runtime-validates every response. Eight migrations and 17 API integration cases pass on fresh PostgreSQL; production deployment and moderation administration remain.
