...
Start with the [backend learning course](docs/learning/README.md): 64 separate chapters explaining NestJS, architecture, PostgreSQL, authentication, cryptography, testing, and deployment through this project's code. Each topic includes mechanisms, concrete examples, failure cases, and exercises with worked reasoning. Follow the reading paths and finish with the NestJS practice labs. Planned queue, realtime, sandbox, and judging features are labeled separately from implemented behavior.
...
Logout, executions and submissions routes require an unexpired server-side session. OIDC login is implemented and remains disabled until a real client registration is configured; there is no demo authentication bypass. See [OIDC setup and provider registration](docs/OIDC_SETUP.md). Execution creation is disabled by default. Durable creation, shared quotas/capacity, version pinning, queued cancellation/expiry and private history are verified, but no queue consumer or runner exists yet. Each API instance performs bounded queued-expiry maintenance at startup and every 15 seconds by default. See [stage 4 configuration and behavior](docs/DURABLE_JOBS.md). Cancellation currently handles queued jobs only; active sandbox cancellation belongs to stage 5/6. Socket.IO delivery is not implemented yet.
...
Foundation, persistence/problems, identity/authorization, and the durable-job API/database milestone are implemented and verified with automated checks. OIDC login issues browser-bound sessions after signed-token validation. Durable admission enforces owner/global active caps and shared new-job quotas; database lifecycle protection and bounded queued expiry preserve terminal outcomes. A live identity provider has not been connected yet; follow the setup guide and smoke-test it before deployment. The sample includes public and hidden test data for judging development; the public problem API queries public examples only.

Next: stage 5—outbox dispatch, BullMQ queue leases/fencing and authenticated Socket.IO delivery/replay, including duplicate delivery and queue failure recovery. Execution stays off until runner isolation and judging pass their launch checks.
