...
| 7. Trusted judging | Public Run / private Submit suites, external comparator, verdicts, sanitized diagnostics, aggregate metrics | Correct/incorrect solutions judged accurately; hidden output never leaks | In progress—worker dependency boundary passes; live judging and measured metrics pending |
...
Current: the stage 6 runtime images, all 14 dedicated-host gVisor isolation tests, the idle supervisor/janitor lifecycle drill, and the restricted worker dependency check pass. The first live-gate attempt correctly exposed that production deployment migrated but did not seed its required immutable fixture; deployment now runs a compiled, repeatable, conflict-detecting seed before activation. Complete the supervisor crash drill, rerun controlled live judging, measured-resource acceptance and independent review before launch. Execution stays off until every runner and judging launch check passes.
