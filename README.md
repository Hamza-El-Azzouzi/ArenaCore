...
| 6. Isolated execution | Dedicated runner, gVisor supervisor, pinned Java/Python/Node images, isolated compilation, resource/output/network limits | All languages run correctly; adversarial isolation tests and cleanup pass | In progress—supervisor implemented; dedicated gVisor host tests pending |
| 7. Trusted judging | Public Run / private Submit suites, external comparator, verdicts, sanitized diagnostics, aggregate metrics | Correct/incorrect solutions judged accurately; hidden output never leaks | In progress—judge/worker protocol implemented; live execution and measured metrics pending |
...
Start with the [backend learning course](docs/learning/README.md): 77 separate chapters explaining NestJS, architecture, PostgreSQL, authentication, cryptography, testing, and deployment through this project's code. Each topic includes mechanisms, concrete examples, failure cases, and exercises with worked reasoning. Follow the reading paths and finish with the NestJS practice labs. Implemented queue/realtime, supervisor and judging protocols are distinguished from pending live-host acceptance.
...
Current: stages 6–7 have supervisor and trusted judging implementations; complete dedicated gVisor host, live judging and measured-resource acceptance before launch. Execution stays off until runner isolation and judging pass their launch checks.
...
- Isolated runner implementation: added separate runner/runtime-policy workspaces, strict digest-only operator image manifest, fixed Java/Python/Node commands, isolated Java compilation and normalized bounded class transfer, per-case fresh sandboxes, fail-closed Docker/gVisor/cgroup/image preflight and effective-policy inspection, CPU/memory/PID/file/scratch/output limits, forced removal with absence verification, intake/drain, bounded private Unix HTTP server/client, and independent janitor/service templates. Cleanup uncertainty now records infrastructure failure rather than successful cancellation. Added 19 policy/artifact tests, four actual Unix RPC tests, one real-worker cleanup-failure test, 14 opt-in host isolation tests, and five detailed learning chapters (72 total). Schema validation, build, full typecheck and 123 locally runnable tests passed. The dedicated ARM64 host now has Docker, cgroup v2 and a working `runsc` runtime. Initial Java, Python and JavaScript candidates were built and published; Java passed the HIGH/CRITICAL vulnerability gate. The first Debian-based Python and JavaScript candidates failed and remain prohibited. Hardened Alpine `arm64-3` replacements remove unused package installers and passed with zero HIGH/CRITICAL OS or language-package findings. Stage 6 remains in progress until those replacements are published by digest and the full live isolation suite, services and measured host acceptance checks pass. Production execution remains disabled.

- Trusted judging implementation: expanded the judge package with strict observation validation, pinned ordered case selection, EXACT_NEWLINE comparison, deterministic per-case/aggregate learner verdicts, compilation/runtime/resource classification and safe bounded Run/Submit projection. Added the Prisma pinned-plan loader, supervisor-backed judging adapter, bounded public RUN console publication and outputTruncated metadata, plus a separate default-disabled/nonproduction-only judging worker entrypoint. Expected answers stay outside supervisor requests and hidden diagnostics are never persisted/published publicly. Added 21 judge/adapter/activation tests and two real PostgreSQL/Redis/BullMQ judging/privacy scenarios with controlled observations; schema, build, full typecheck and all 146 locally runnable tests passed, while 14 gVisor host tests remain unrun. Dependency audit reported zero known vulnerabilities. Added the stage 7 guide and five deep concept chapters (77 total). Stage 7 remains in progress for live language judging and measured CPU/peak-memory/OOM evidence; missing metrics are omitted, not fabricated. Stage 6 host/image/cleanup and production launch gates still apply.
