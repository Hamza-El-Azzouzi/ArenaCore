...
| 6. Isolated execution | Dedicated runner, gVisor supervisor, pinned Java/Python/Node images, isolated compilation, resource/output/network limits | All languages run correctly; adversarial isolation tests and cleanup pass | In progress—host isolation/lifecycle pass; crash verifier ready; host run/review pending |
| 7. Trusted judging | Public Run / private Submit suites, external comparator, verdicts, sanitized diagnostics, aggregate metrics | Correct/incorrect solutions judged accurately; hidden output never leaks | In progress—live three-language judging and privacy pass; measured metrics pending |
...
| 9. Production operations | IaC, CI/CD, staging, secrets, telemetry, backups, restore, rollback, capacity and abuse limits | Security review, load test, restore and rollout drills pass | In progress—verified-commit SSH deployment implemented; host bootstrap and rollout drills pending |
...
Start with the [backend learning course](docs/learning/README.md): 80 separate chapters explaining NestJS, architecture, PostgreSQL, authentication, cryptography, testing, and deployment through this project's code. Each topic includes mechanisms, concrete examples, failure cases, and exercises with worked reasoning. Follow the reading paths and finish with the NestJS practice labs. Implemented queue/realtime, supervisor and judging protocols are distinguished from pending live-host acceptance.
...
Current: the stage 6 runtime images, all 14 dedicated-host gVisor isolation tests, the idle supervisor/janitor lifecycle drill, restricted worker dependency check, and controlled seven-job live judging gate pass. Correct Run and Submit completed for Python, JavaScript and Java; the wrong hidden-output Submit persisted no hidden diagnostics; all acceptance data was removed; the worker returned to inactive/disabled with no managed sandbox. The idle-only supervisor crash verifier is implemented and awaits its runner-host result. Complete that run, measured-resource acceptance and independent review before launch. Execution stays off until every runner and judging launch check passes.
...
- Isolated runner implementation: added separate runner/runtime-policy workspaces, strict digest-only operator image manifest, fixed Java/Python/Node commands, isolated Java compilation and normalized bounded class transfer, per-case fresh sandboxes, fail-closed Docker/gVisor/cgroup/image preflight and effective-policy inspection, CPU/memory/PID/file/scratch/output limits, forced removal with absence verification, intake/drain, bounded private Unix HTTP server/client, and independent janitor/service templates. Cleanup uncertainty records infrastructure failure rather than successful cancellation. The dedicated ARM64 host now has Docker, cgroup v2, a verified `runsc` runtime and approved digest-only Java/Python/JavaScript images. Java and the hardened Alpine `arm64-3` Python/JavaScript images passed the HIGH/CRITICAL vulnerability gate. All 14 live functional and adversarial gVisor tests pass, including language execution, network/metadata denial, read-only roots, fresh scratch, memory/PID/output/storage limits and cleanup. The idle systemd lifecycle drill also passed, proving graceful supervisor restart, socket permissions/protocol and independent expired-sandbox cleanup. Added an idle-only abrupt-death verifier that creates a real `runsc` sandbox, SIGKILLs the supervisor, requires deadline-based janitor cleanup while it remains dead, then explicitly restores and probes the service. Its production-host run, measured resource evidence and independent review remain. Production execution remains disabled.

- Trusted judging implementation: expanded the judge package with strict observation validation, pinned ordered case selection, EXACT_NEWLINE comparison, deterministic per-case/aggregate learner verdicts, compilation/runtime/resource classification and safe bounded Run/Submit projection. Added the Prisma pinned-plan loader, supervisor-backed judging adapter, bounded public RUN console publication and outputTruncated metadata, plus a separate default-disabled production worker with worker-only configuration, a Docker-denied service identity and a non-consuming dependency-check service. Expected answers stay outside supervisor requests and hidden diagnostics are never persisted/published publicly. The dedicated host printed `WORKER_INSTALLATION_CHECK_PASSED`, proving the restricted identity could authenticate to PostgreSQL/Redis, held the exact table privileges and reached the private supervisor socket without joining the Docker group. The operator-only live gate then passed seven real Run/Submit jobs across all languages, including hidden-output non-disclosure, fenced terminal results, scoped queue/database cleanup, and a final inactive/disabled worker with no managed sandbox. Stage 7 remains in progress for measured CPU/peak-memory/OOM evidence. Stage 6 crash/review and production launch gates still apply.
