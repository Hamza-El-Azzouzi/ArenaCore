...
An exit code of 137 does not independently prove OOM. `MEMORY_LIMIT_EXCEEDED` is emitted when the live case cgroup's `oom_kill` counter increases. If OOM kills the entire sandbox and removes its live PID before the final snapshot, Docker's persisted `State.OOMKilled=true` is accepted as terminal resource evidence; exit status alone remains insufficient. CPU runtime is the case's `usage_usec` delta and memory is that fresh sandbox's `memory.peak`, rounded up to KiB. Terminal OOM may omit the metric pair because its final cgroup files no longer exist. The judge exposes per-case values only for public Run cases and persists aggregate maxima only when every observed case has a complete pair.
