...
The worker accepts only an explicit production configuration with `RUNNER_WORKER_ENABLED=true`. Its systemd unit remains disabled until the remaining launch gates pass. The expanded 15-test dedicated gVisor suite, idle supervisor/janitor lifecycle drill, restricted dependency check, controlled seven-job live judging gate, abrupt supervisor-death gate, and restricted-identity cgroup metric gate pass. Stage 7 is complete; independent review and production operations remain. This milestone alone does not clear public launch gates.
...
An exit code of 137 does not independently prove OOM. `MEMORY_LIMIT_EXCEEDED` is emitted when the live case cgroup's `oom_kill` counter increases. If OOM kills the entire sandbox and removes its live PID before the final snapshot, Docker's persisted `State.OOMKilled=true` is accepted as terminal resource evidence; exit status alone remains insufficient. A strictly stopped non-OOM container can retain its completed command result but omits the unavailable metric pair. A metric failure while Docker still reports a live PID remains an infrastructure failure. The judge exposes per-case values only for public Run cases and persists aggregate maxima only when every observed case has a complete pair.
...
Current verification: schema validation, workspace build, full typecheck and 65 non-integration tests passed locally. The service-identity metric gate and all 15 expanded live-host tests passed on the dedicated runner. No database migration was needed for metric collection.
