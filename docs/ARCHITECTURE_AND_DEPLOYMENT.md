...
| Database | PostgreSQL, Prisma migrations | Durable jobs, versioned problems/tests, submissions, idempotency, audit events |
| Queue / transient events | Redis, BullMQ | Bounded work queue, worker leases, bounded public event replay |
...
2. Client subscribes to an execution room; gateway checks ownership on every subscribe/replay/cancel request. Queue dispatch does not depend on the browser connection.
...
5. Persist the authoritative terminal result before publishing final_verdict. Duplicate deliveries may run again after a failure but only the current fenced attempt can commit; public events also carry the attempt. Terminal states never regress.
...
With multiple Socket.IO gateway instances, configure an adapter to relay events and sticky sessions if HTTP long-polling is enabled; a Redis queue is not itself a Socket.IO adapter. See [multi-node guidance](https://socket.io/docs/v4/using-multiple-nodes/). Durable public replay is still application-managed.
