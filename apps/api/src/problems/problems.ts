...
const publicVersionSelect = {
  ...
  timeMs: true, memoryKiB: true, templates: true,
  testCases: {where: {visibility: 'PUBLIC' as const}, orderBy: {ordinal: 'asc' as const}, select: {id: true, input: true, expectedOutput: true}},
...
export class Problems {
  ...
  async detail(slug: string): Promise<ProblemDetail> {
    ...
    return {id: row.id, slug: row.slug, title: v.title, difficulty: v.difficulty, tags: v.tags, statementMarkdown: v.statementMarkdown, constraints: v.constraints, limits: {timeMs: v.timeMs, memoryKiB: v.memoryKiB}, examples: v.testCases.map(t => ({id: t.id, input: t.input, expectedOutput: t.expectedOutput})), templates: templatesSchema.parse(v.templates)};
  }
}
@Controller('problems')
