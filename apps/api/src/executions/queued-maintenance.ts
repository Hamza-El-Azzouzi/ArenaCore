...
export async function expireQueuedJobs(db: PrismaClient): Promise<number> {
  return db.$transaction(async tx => {
    const now = new Date();
    const expired = await tx.$queryRaw<Execution[]>`
    ...
    const jobs = new JobStore(db as Database);
    for (const row of expired) {
      const sequence = await jobs.event(tx, row, {kind: 'final_verdict', state: 'INTERNAL_ERROR', verdict: 'INTERNAL_ERROR', failureCode: 'QUEUE_TIMEOUT'});
      await tx.execution.update({where: {id: row.id}, data: {
        state: 'INTERNAL_ERROR', verdict: 'INTERNAL_ERROR', failureCode: 'QUEUE_TIMEOUT', finishedAt: now, lastSequence: sequence,
      }});
    }
