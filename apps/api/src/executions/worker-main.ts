...
async function main(){
  ...
  const client=new SupervisorClient(config.RUNNER_SOCKET_PATH),db=new Database({values:{DATABASE_URL:config.DATABASE_URL}});await db.$connect();
  if(process.argv[2]==='--check') {
    ...
    try {
      ...
      await Promise.all([within(queue.waitUntilReady(),10000),checkSupervisor(config.RUNNER_SOCKET_PATH)]);console.log('WORKER_DEPENDENCY_CHECK_PASSED');
    }
