...
export class ExecutionRealtime implements OnApplicationBootstrap,OnModuleDestroy {
  ...
  private readonly clients=new Map<Socket,{userId:string;subscriptions:Map<string,Cursor>;busy:boolean;requests:number;window:number}>();
  ...
  onApplicationBootstrap() {
    ...
    ns.on('connection',socket=>{
      const client={userId:socket.data.userId as string,subscriptions:new Map<string,Cursor>(),busy:false,requests:0,window:Date.now()};
      ...
      socket.on('subscribe_execution',async(input:unknown,ack:unknown)=>{
        ...
        if (++client.requests>60 || client.busy) return reply({ok:false,error:{code:'SOCKET_RATE_LIMIT'}});
        ...
        } catch(e) {reply({ok:false,error:{code:e instanceof ApiError?'NOT_FOUND':'SERVICE_UNAVAILABLE'}});} finally {client.busy=false;}
      });
  ...
  private async poll() {
    for (const [socket,client] of this.clients) {
      ...
      try {
        const principal=await this.sessions.resolve(socket.request as Request);
        if (!principal || principal.userId!==client.userId) {socket.disconnect(true);continue;}
      ...
      } catch {socket.disconnect(true);} finally {client.busy=false;}
    }
  }
