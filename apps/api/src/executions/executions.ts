...
export class Executions {
  ...
  async cancel(userId: string, id: string): Promise<ExecutionReceipt> {
    ...
    return this.db.$transaction(async tx => {
      ...
      const row = await tx.execution.findFirst({where: {id, userId}, select: {id: true, state: true}});
      if (!row) throw new ApiError(404, 'NOT_FOUND', 'Execution not found.');
      if (row.state === 'COMPILING' || row.state === 'RUNNING') throw new ApiError(503, 'CANCELLATION_UNAVAILABLE', 'Active runner cancellation is not available yet.');
      if (changed.count) await tx.outboxEvent.create({data: {kind: 'EXECUTION_CANCELLED', executionId: id}});
      return {executionId: row.id, state: row.state};
    });
  }
