...
export class Competitions {
  ...
  private async managed(userId:string,role:'USER'|'ADMIN',slug:string){const row=await this.db.competition.findUnique({where:{slug},select:{id:true,ownerId:true,startsAt:true,endsAt:true}});if(!row)throw new ApiError(404,'NOT_FOUND','Competition not found.');if(role!=='ADMIN'&&row.ownerId!==userId)throw new ApiError(403,'ORGANIZER_REQUIRED','Only this competition organizer or an administrator can manage it.');return row;}
  ...
  async mine(userId:string){const rows=await this.db.competition.findMany({where:{ownerId:userId},select:{id:true,slug:true,kind:true,title:true,published:true,startsAt:true,endsAt:true,_count:{select:{registrations:true,rounds:true}}},orderBy:[{startsAt:'desc'},{id:'desc'}]});return {items:rows.map(row=>({...row,startsAt:row.startsAt.toISOString(),endsAt:row.endsAt.toISOString(),status:status(row.startsAt,row.endsAt),registrations:row._count.registrations,rounds:row._count.rounds,_count:undefined}))};}

  async management(userId:string,role:'USER'|'ADMIN',slug:string){await this.managed(userId,role,slug);const row=await this.db.competition.findUniqueOrThrow({where:{slug},select:{id:true,slug:true,kind:true,title:true,description:true,rulesMarkdown:true,prizeLabel:true,published:true,ownerId:true,startsAt:true,endsAt:true,_count:{select:{registrations:true}},rounds:{orderBy:{ordinal:'asc'},select:{id:true,title:true,ordinal:true,startsAt:true,endsAt:true,problems:{orderBy:{ordinal:'asc'},select:{ordinal:true,points:true,problem:{select:{id:true,slug:true,currentVersion:{select:{title:true,difficulty:true,tags:true}}}}}}}}}});return {...row,startsAt:row.startsAt.toISOString(),endsAt:row.endsAt.toISOString(),status:status(row.startsAt,row.endsAt),registrations:row._count.registrations,_count:undefined,rounds:row.rounds.map(round=>({...round,startsAt:round.startsAt.toISOString(),endsAt:round.endsAt.toISOString(),status:status(round.startsAt,round.endsAt),problems:round.problems.flatMap(item=>item.problem.currentVersion?[{id:item.problem.id,slug:item.problem.slug,title:item.problem.currentVersion.title,difficulty:item.problem.currentVersion.difficulty,tags:item.problem.currentVersion.tags,points:item.points,ordinal:item.ordinal}]:[])}))};}

  async registrations(userId:string,role:'USER'|'ADMIN',slug:string){const competition=await this.managed(userId,role,slug);const rows=await this.db.competitionRegistration.findMany({where:{competitionId:competition.id},select:{joinedAt:true,user:{select:{username:true,displayName:true}}},orderBy:[{joinedAt:'asc'},{userId:'asc'}]});return {items:rows.map(row=>({username:row.user.username,displayName:row.user.displayName,joinedAt:row.joinedAt.toISOString()}))};}

  async submissions(userId:string,role:'USER'|'ADMIN',slug:string){const competition=await this.managed(userId,role,slug);const rows=await this.db.execution.findMany({where:{mode:'SUBMIT',competitionRound:{competitionId:competition.id}},select:{id:true,state:true,verdict:true,language:true,runtimeMs:true,memoryKiB:true,createdAt:true,user:{select:{username:true,displayName:true}},competitionRound:{select:{title:true}},problemVersion:{select:{title:true,problem:{select:{slug:true}}}}},orderBy:[{createdAt:'desc'},{id:'desc'}],take:200});return {items:rows.map(row=>({id:row.id,state:row.state,verdict:row.verdict,language:row.language,runtimeMs:row.runtimeMs,memoryKiB:row.memoryKiB,createdAt:row.createdAt.toISOString(),user:row.user,round:row.competitionRound!.title,problem:{slug:row.problemVersion.problem.slug,title:row.problemVersion.title}}))};}

  async update(userId:string,role:'USER'|'ADMIN',slug:string,input:z.infer<typeof createSchema>){const current=await this.managed(userId,role,slug);if(input.slug!==slug)throw new ApiError(409,'SLUG_IMMUTABLE','A competition slug cannot change after creation.');if(new Date()>=current.startsAt)throw new ApiError(409,'COMPETITION_STARTED','Competition structure cannot change after it starts.');const startsAt=new Date(input.startsAt),endsAt=new Date(input.endsAt),now=new Date();if(startsAt.getTime()<now.getTime()+60_000||endsAt<=startsAt)throw new ApiError(400,'INVALID_SCHEDULE','The updated competition must start in the future and end after it starts.');if(input.kind==='CONTEST'&&input.rounds.length!==1)throw new ApiError(400,'INVALID_ROUNDS','A contest must contain exactly one round.');for(const round of input.rounds){const start=new Date(round.startsAt),end=new Date(round.endsAt);if(start<startsAt||end>endsAt||end<=start)throw new ApiError(400,'INVALID_SCHEDULE','Every round must fit inside the competition schedule.');}const slugs=[...new Set(input.rounds.flatMap(round=>round.problemSlugs))];const problems=await this.db.problem.findMany({where:{slug:{in:slugs},currentVersion:{published:true}},select:{id:true,slug:true}});if(problems.length!==slugs.length)throw new ApiError(400,'INVALID_PROBLEM','Every round problem must reference a published problem.');const ids=new Map(problems.map(problem=>[problem.slug,problem.id]));return this.db.$transaction(async tx=>{await tx.competitionRound.deleteMany({where:{competitionId:current.id}});const competition=await tx.competition.update({where:{id:current.id},data:{kind:input.kind,title:input.title,description:input.description,rulesMarkdown:input.rulesMarkdown,prizeLabel:input.prizeLabel,startsAt,endsAt},select:{id:true,slug:true,kind:true}});for(const [index,round] of input.rounds.entries()){const created=await tx.competitionRound.create({data:{competitionId:current.id,title:round.title,ordinal:index+1,startsAt:new Date(round.startsAt),endsAt:new Date(round.endsAt)}});await tx.competitionProblem.createMany({data:round.problemSlugs.map((problemSlug,ordinal)=>({roundId:created.id,problemId:ids.get(problemSlug)!,ordinal:ordinal+1,points:100}))});}await tx.auditEvent.create({data:{actorId:userId,action:role==='ADMIN'?'ADMIN_COMPETITION_UPDATE':'OWNER_COMPETITION_UPDATE',targetId:current.id}});return competition;});}
  ...
  async leaderboard(slug:string) {
    ...
    if(!competition)throw new ApiError(404,'NOT_FOUND','Competition not found.');
    return this.buildLeaderboard(competition);
  }

  async managedLeaderboard(userId:string,role:'USER'|'ADMIN',slug:string){const managed=await this.managed(userId,role,slug);const competition=await this.db.competition.findUniqueOrThrow({where:{id:managed.id},select:{id:true,startsAt:true,endsAt:true,rounds:{select:{problems:{select:{problemId:true,points:true}}}}}});return this.buildLeaderboard(competition);}

  private async buildLeaderboard(competition:{id:string;startsAt:Date;endsAt:Date;rounds:{problems:{problemId:string;points:number}[]}[]}) {
...
export class CompetitionsController {
  ...
  @Get('mine') @UseGuards(SessionGuard) mine(@Req() req:AuthenticatedRequest){return this.competitions.mine(req.principal.userId);}
  @Get(':slug/manage') @UseGuards(SessionGuard) management(@Req() req:AuthenticatedRequest,@Param('slug') slug:string){return this.competitions.management(req.principal.userId,req.principal.role,validate(slugSchema,slug));}
  @Get(':slug/manage/leaderboard') @UseGuards(SessionGuard) managedLeaderboard(@Req() req:AuthenticatedRequest,@Param('slug') slug:string){return this.competitions.managedLeaderboard(req.principal.userId,req.principal.role,validate(slugSchema,slug));}
  @Get(':slug/registrations') @UseGuards(SessionGuard) registrations(@Req() req:AuthenticatedRequest,@Param('slug') slug:string){return this.competitions.registrations(req.principal.userId,req.principal.role,validate(slugSchema,slug));}
  @Get(':slug/submissions') @UseGuards(SessionGuard) submissions(@Req() req:AuthenticatedRequest,@Param('slug') slug:string){return this.competitions.submissions(req.principal.userId,req.principal.role,validate(slugSchema,slug));}
  ...
  @Patch(':slug') @UseGuards(SessionGuard)
  update(@Req() req:AuthenticatedRequest,@Param('slug') slug:string,@Body() body:unknown){return this.competitions.update(req.principal.userId,req.principal.role,validate(slugSchema,slug),validate(createSchema,body));}
