...
export class LoginService {
  constructor(@Inject(Database) private readonly db: Database, @Inject(Config) private readonly config: Config, @Inject(OidcGateway) private readonly oidc: OidcGateway) {}
  private key() {
    if (!this.config.oidcEnabled) throw new ApiError(503, 'IDENTITY_NOT_CONFIGURED', 'Sign-in is not available yet.');
    return Buffer.from(this.config.values.OIDC_TRANSACTION_KEY!, 'base64');
  }
  ...
  async begin(req: Request) {
    const key = this.key();
    if ((req.headers.origin && req.headers.origin !== this.config.values.PUBLIC_ORIGIN) || req.headers['sec-fetch-site'] === 'cross-site') {
    ...
    const location = await this.oidc.authorizationUrl(state, proof);
  ...
  async finish(req: Request, params: Record<string, string>) {
    ...
    const identity = await this.oidc.exchange(callback, state, proof);
    ...
    await this.db.$transaction(async tx => {
      ...
      const active = await tx.session.findMany({where: {userId: user.id, revokedAt: null, expiresAt: {gt: new Date()}}, orderBy: [{createdAt: 'desc'}, {id: 'desc'}], select: {id: true}});
      if (active.length >= 10) await tx.session.updateMany({where: {id: {in: active.slice(9).map(s => s.id)}}, data: {revokedAt: new Date()}});
      await tx.session.create({data: {userId: user.id, tokenHash: session.tokenHash, csrfTokenHash: session.csrfTokenHash, expiresAt}});
      await tx.auditEvent.create({data: {actorId: user.id, action: 'AUTH_LOGIN', targetId: user.id}});
    });
